...
begin
  ...
  begin
    s := Format('col %d row %d index %d', [Col, Row, Index]);
    edit1.Text := s;
    if s <>  PaintBox1.Hint then
    begin
      Application.HideHint; //need to hide only if hint changed
      PaintBox1.Hint := s;
    end;
  end;
end;
...
begin
  if PointToIndex(X, Y, Col, Row, Index) then
  begin
    Shape1.Brush.Color := BrightColorArray[Index];
  end;
end;
